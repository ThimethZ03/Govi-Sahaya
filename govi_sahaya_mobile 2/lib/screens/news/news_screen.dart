...
class _NewsScreenState extends State<NewsScreen>
  ...
  Widget build(BuildContext context) {
    ...
    // ✅ Filter by agriculture keywords OR category
    final filtered = newsProvider.newsList.where((n) {
      // Filter by agriculture keywords if selected category is 'all' or 'agriculture'
      final isAgriNews = _isAgriculturalNews(n);

      if (_selectedCategory == 'all') {
        return isAgriNews; // Show only agriculture news when 'All' is selected
      }

      // Otherwise apply category filter
      final cat = (n.category).toLowerCase().trim();
      final sel = _selectedCategory.toLowerCase();
      final categoryMatch = cat == sel ||
          cat.replaceAll('_', '') == sel.replaceAll('_', '') ||
          cat.startsWith(sel) ||
          sel.startsWith(cat);

      // If category is agriculture-specific, apply both filters
      if (['technology', 'market_prices', 'weather', 'government_policy']
          .contains(sel)) {
        return categoryMatch && isAgriNews;
      }
  ...
  bool _isAgriculturalNews(dynamic news) {
    // Agriculture-related keywords in English and Sinhala
    const agriKeywordsEn = [
      'rice',
      'paddy',
      'grain',
      'crop',
    ...
    return agriKeywordsEn.any((keyword) => content.contains(keyword));
  }
